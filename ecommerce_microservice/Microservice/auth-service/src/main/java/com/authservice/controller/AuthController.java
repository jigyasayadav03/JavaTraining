...
//	}
	...
	public ResponseEntity<User> getUserByID(@PathVariable Long id, Authentication authentication) {
		...
		if (authentication != null && authentication.isAuthenticated()) {
			if (authentication.getAuthorities().stream().anyMatch(r -> r.getAuthority().equals("ADMIN_ROLES"))) {
				return ResponseEntity.ok(userInfoService.getUserByUserId(id));
			} else {
				String u_name = filter.getuname();
				User user = userInfoService.getUserByUserId(id);
				if (u_name != null) {
	...
	public ResponseEntity<List<User>> getAllUser() {
		return new ResponseEntity<List<User>>(userInfoService.getAllUsers(), HttpStatus.OK);
	}
...
//	}
	...
	public ResponseEntity<User> updateUser(@RequestBody User user) {
		User userResponse = userInfoService.updateUser(user);
	...
	public ResponseEntity<User> getUserByWithCartId(@PathVariable Long id) {
		User u = userInfoService.getUserBywithcartInfo(id);
